...
import { Building2, Home, Factory, Landmark, Wrench, ClipboardList, CheckCircle2 } from 'lucide-react';
...
export default function ServicesPage() {
  ...
  return (
    <main className="bg-stone-950 pt-20">
      {/* Hero */}
      <section ref={heroRef} className="relative py-24 lg:py-28 overflow-hidden">
        <div className="absolute inset-0 grid-bg opacity-20" />
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-orange-500" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div data-a className="inline-flex items-center gap-2 text-orange-500 mb-4">
            <div className="h-px w-8 bg-orange-500" />
            <span className="font-heading font-semibold text-sm tracking-widest uppercase">What We Offer</span>
          </div>
          ...
          <p data-a className="text-stone-300 text-xl max-w-2xl leading-relaxed">
            End-to-end construction solutions built on 25 years of technical expertise and a culture of delivery excellence.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 lg:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          {services.map((service, i) => (
            <ServiceBlock key={service.id} service={service} reverse={i % 2 !== 0} />
          ))}
        </div>
      </section>
...
function ServiceBlock({ service, reverse }) {
  ...
  return (
    ...
    >
      {/* Image */}
      <div data-b className={`aspect-[16/9] lg:aspect-auto lg:h-full min-h-[280px] overflow-hidden relative ${reverse ? 'lg:[direction:ltr]' : ''}`}>
        <img
          src={service.image}
          alt={service.title}
      ...
      <div className={`p-8 lg:p-10 ${reverse ? 'lg:[direction:ltr]' : ''}`}>
        <div data-b className="w-12 h-12 bg-orange-500/10 border border-orange-500/20 rounded-sm flex items-center justify-center mb-5">
          <Icon className="w-6 h-6 text-orange-500" />
        </div>
        <h2 data-b className="font-heading font-bold text-3xl lg:text-4xl text-white mb-2">{service.title}</h2>
        <p data-b className="text-orange-400 font-heading text-sm tracking-widest uppercase mb-4">{service.tagline}</p>
        <p data-b className="text-stone-400 leading-relaxed mb-6">{service.description}</p>
